function appearlayer1(){
	document.getElementById("layer4").style.visibility="hidden";
	document.getElementById("layer2").style.visibility="hidden";
	document.getElementById("layer3").style.visibility="hidden";
	document.getElementById("layer1").style.visibility="visible";
	document.getElementById("layer4").style.height="0%";
	document.getElementById("layer3").style.height="0%";
	document.getElementById("layer2").style.height="0%";
	document.getElementById("layer1").style.height="100%";


}function appearlayer2(){
	document.getElementById("layer1").style.visibility="hidden";
	document.getElementById("layer4").style.visibility="hidden";
	document.getElementById("layer3").style.visibility="hidden";
	document.getElementById("layer2").style.visibility="visible";
	document.getElementById("layer4").style.height="0%";
	document.getElementById("layer3").style.height="0%";
	document.getElementById("layer1").style.height="0%";
	document.getElementById("layer2").style.height="100%";



}function appearlayer3(){
	document.getElementById("layer1").style.visibility="hidden";
	document.getElementById("layer2").style.visibility="hidden";
	document.getElementById("layer4").style.visibility="hidden";
	document.getElementById("layer3").style.visibility="visible";
	document.getElementById("layer4").style.height="0%";
	document.getElementById("layer1").style.height="0%";
	document.getElementById("layer2").style.height="0%";
	document.getElementById("layer3").style.height="100%";


}
function appearlayer4(){
	document.getElementById("layer1").style.visibility="hidden";
	document.getElementById("layer1").style.height="0%";
	document.getElementById("layer2").style.visibility="hidden";
	document.getElementById("layer2").style.height="0%";
	document.getElementById("layer3").style.visibility="hidden";
	document.getElementById("layer3").style.height="0%";
	document.getElementById("layer4").style.visibility="visible";
	document.getElementById("layer4").style.height="100%";

}